...
(function(window, $, undefined) {
  ...
  $.fn.sudokuJS = function(opts) {
    ...
    var initBoard = function(opts) {
      ...
      boardNumbers = [];
      boardSize =
        (!board.length && opts.boardSize) || Math.sqrt(board.length) || 9;
      ...
      if (!alreadyEnhanced) {
        ...
        for (var j = 0; j < boardSize * boardSize; j++) {
          ...
          var candidates =
            cellVal === null ? boardNumbers.slice() : nullCandidateList.slice();
          board[j] = {
            val: cellVal,
    ...
    var uIBoardHighlightRemoveCandidate = function(cellIndex, digit) {
      $(
        "#input-" + cellIndex + "-candidates div:nth-of-type(" + digit + ")"
      ).addClass("candidate--to-remove");
    };
        ...
        if (board[i].val === null) {
          ...
          if (updateUI !== false)
            $("#input-" + i + "-candidates").html(
              buildCandidatesString(resetCandidatesList)
            );
    ...
    var housesWithCell = function(cellIndex) {
      ...
      //box
      var box =
        Math.floor(hrow / boxSideSize) * boxSideSize +
        Math.floor(vrow / boxSideSize);
      houses.push(box);
        ...
        for (var i = 0; i < boardSize; i++) {
          ...
          for (var j = 0; j < digits.length; j++) {
            ...
            if (
              ...
              var cellsUpdated = removeCandidatesFromCells(cellsEffected, [
                digit
              ]);
    ...
    function nakedCandidates(n) {
      ...
      function checkCombinedCandidates(house, startIndex) {
        ...
        for (
          var i = Math.max(startIndex, minIndexes[startIndex]);
          i < boardSize - n + startIndex;
          i++
        ) {
          //log(i);
          ...
          if (combineInfo.length === n) {
            ...
            var cellsUpdated = removeCandidatesFromCells(
              cellsEffected,
              combinedCandidates
            );
    ...
    var hardEnough = function(data) {
      ...
      if (difficulty === DIFFICULTY_HARD)
        return (
          data.level !== DIFFICULTY_EASY && data.level !== DIFFICULTY_MEDIUM
        );
      if (difficulty === DIFFICULTY_VERY_HARD)
        return (
          data.level !== DIFFICULTY_EASY &&
          data.level !== DIFFICULTY_MEDIUM &&
          data.level !== DIFFICULTY_HARD
        );
    };
